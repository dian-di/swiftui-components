...
struct ClockView: View {
        ...
        ZStack {
            ...
            GeometryReader { proxy in
                ...
                ZStack {
                        ...
                        ZStack {
                            if index % 2 == 0 {
                                Text("\(index == 24 ? 0 : index)")
                                    .font(.callout.bold())
                                    .foregroundColor(
                                        index % 6 == 0 ? .white : .gray
                                    )
                                    .rotationEffect(
                                        .init(
                                            degrees: Double(-180 - index * 15)
                                        )
                                    )
                                    .offset(y: (width - 100) / 2)
                                    .rotationEffect(
                                        .init(degrees: Double(index) * 15)
                                    )
                            }
                        }
                    ...
                    // 可动态调整的圆环部分
                    let reverseRotation =
                        (startProgress > toProgress)
                        ? -Double((1 - startProgress) * 360) : 0
                    Circle()
                        .trim(
                            from: startProgress > toProgress
                                ? 0 : startProgress,
            ...
            VStack {
                ...
                HStack {
                    VStack {
                        ...
                        Text(
                            getTime(angle: startAngle).formatted(
                                date: .omitted,
                                time: .shortened
                            )
                        )
                    ...
                    VStack {
                        ...
                        Text(
                            getTime(angle: toAngle).formatted(
                                date: .omitted,
                                time: .shortened
                            )
                        )
                ...
                HStack {
                    Text(
                        "\(getTimeDifference().0) h"
                            + "  \(getTimeDifference().1) min"
                    )
                    .font(.title2.bold())
                }
                .padding(.vertical)
    ...
    func getTimeDifference() -> (Int, Int) {
        ...
        var hour =
            (result.hour ?? 0) < 0
            ? (result.hour ?? 0) + 24 : (result.hour ?? 0)
        var minute =
            (result.minute ?? 0) < 0
            ? (result.minute ?? 0) + 60 : (result.minute ?? 0)
